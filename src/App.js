...
const getAverageJoyRatingForAssignment = (data, assignmentName) => {
  const assScrum = data.filter(item => {
    return item.assignment.includes(assignmentName);
  });
  const joyRateScrum = assScrum.map(item => {
    // array met scrumopdr
    return item.joyRating;
  });
  const allJoyRatingsScrum = assScrum.reduce(
    //nu waardes optellen van joyRate scrum opdrachten
    (previousValue, currentValue) => previousValue + currentValue.joyRating,
    0
  );
  const averageJoyRatingsScrum = // gemiddelde berekenen van joyRating opdracht SCRUM;
    assScrum.reduce(
      (previousValue, currentValue) => previousValue + currentValue.joyRating,
      0
    ) / assScrum.length;
